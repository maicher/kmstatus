...
DirectMap1G:    25165824 kB`)
	...
	if mem.MemTotal != 32814068 {
		t.Fatalf("MemTotal: %d, want: %d", mem.MemTotal, 32814068)
	}

	if mem.MemUsed != 26571588 {
		t.Fatalf("MemUsed: %d, want: %d", mem.MemUsed, 26571588)
	}
	if mem.SwapTotal != 50331644 {
		t.Fatalf("SwapTotal: %d, want: %d", mem.SwapTotal, 50331644)
	}

	if mem.SwapUsed != 3073024 {
		t.Fatalf("SwapUsed: %d, want: %d", mem.SwapUsed, 3073024)
	}
}
